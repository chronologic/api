...
export default function createClass<ApiType extends ApiTypes> ({
  api,
  apiType,
  decorateMethod
  ...
  return class Submittable extends Extrinsic implements SubmittableExtrinsic<ApiType> {
    ...
    ): SubmittablePaymentResult<ApiType> {
      ...
      return this._decorateMethod(
        (): Observable<RuntimeDispatchInfo> =>
          this._getPrelimState(address, allOptions).pipe(
            first(),
            switchMap(
              ([nonce, header]): Observable<RuntimeDispatchInfo> => {
    ...
    public signAsync (
      account: IKeyringPair | string | AccountId | Address,
      optionsOrNonce: Partial<SignerOptions>
    ): SubmittableThis<ApiType, this> {
      return this._decorateMethod(
        (): Observable<this> => {
          const optionsWithNonce = this._optionsOrNonce(optionsOrNonce);
          const address = isKeyringPair(account) ? account.address : account.toString();

          return this._getPrelimState(address, optionsWithNonce).pipe(
            first(),
            mergeMap(
              ([nonce, header]): Observable<this> => {
                const eraOptions = this._makeEraOptions(optionsWithNonce, { header, nonce });

                return from(
                  isKeyringPair(account)
                    ? Promise.resolve(this.sign(account, eraOptions))
                    : this._signViaSigner(account, eraOptions, header)
                ).pipe(mapTo(this));
              }
            )
          );
    ...
    ): SubmittableResultResult<ApiType> | SubmittableResultSubscription<ApiType> {
    ...
    private _statusObservable (status: ExtrinsicStatus): Observable<SubmittableResultImpl> {
      ...
      return combineLatest([this._api.rpc.chain.getBlock(blockHash), this._api.query.system.events.at(blockHash)]).pipe(
        map(
    ...
    private _optionsOrNonce = (optionsOrNonce: Partial<SignerOptions>): Partial<SignerOptions> => {
      return isBn(optionsOrNonce) || isNumber(optionsOrNonce) ? { nonce: optionsOrNonce } : optionsOrNonce;
    };
  };
}
